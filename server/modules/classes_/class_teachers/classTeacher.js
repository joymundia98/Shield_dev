...
const ClassTeachersModel = {
  async create(class_id, teacher_id) {
  ...
  async findAll() {
    const result = await pool.query(`
      SELECT ct.id, ct.class_id, ct.teacher_id, c.name AS class_name, t.full_name AS teacher_name
  ...
  async update(id, class_id, teacher_id) {
    const result = await pool.query(
