    ...
    const downloadFile = async (type: "pdf" | "excel" | "csv") => {
      try {
        const response = await axios.get(
          `${baseURL}/api/reports/expenses/${type}`,
          {
        ...
        link.download = `expenses_data.${extensionMap[type]}`;
      ...
      <div className="dashboard-content">
        ...
        {Object.entries(groupedExpenses).map(([deptName, groups]) => (
          <div key={deptName}>
            ...
            {groups.length > 0 ? (
              groups.map((group) => {
                const groupKey = `${deptName}-${group.name}`;
                  ...
                  <div key={group.name}>
                    ...
                    <div className="table-buttons">
                      {visibleCount < totalItems && (
                        <button onClick={() => handleViewMore(groupKey, totalItems)}>View More</button>
                      )}
                      {visibleCount > 3 && (
                        <button onClick={() => handleViewLess(groupKey)}>View Less</button>
                      )}
                      {visibleCount !== totalItems && visibleCount > 3 && (
                        <button onClick={() => handleViewAll(groupKey, totalItems)}>View All</button>
                      )}
                      {visibleCount === totalItems && (
                        <button onClick={() => handleBackToInitial(groupKey)}>Back to Initial</button>
                      )}
                    </div>
                  </div>
                );
