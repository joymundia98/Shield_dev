...
const EditPayroll: React.FC = () => {
  ...
  // Fetch payroll data for editing based on payrollId
  useEffect(() => {
    if (!payrollId) return;
    const fetchPayrollData = async () => {
      try {
        ...
        const data = await response.json();
        setForm(data); // Prefill the form with the fetched data
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to update payroll: ${responseData.message || 'Unknown error'}`);
      }

      alert("Payroll updated successfully!");
      navigate("/hr/payroll");
    } catch (err: any) {
      console.error("Error occurred while updating payroll:", err);
